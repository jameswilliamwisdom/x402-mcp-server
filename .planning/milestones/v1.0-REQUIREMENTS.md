# Requirements Archive: v1.0 npm Publish + Brand Site

**Archived:** 2026-03-12
**Status:** SHIPPED

For current requirements, see `.planning/REQUIREMENTS.md`.

---

# Requirements: x402 API Network

**Defined:** 2026-03-09
**Core Value:** AI agents can discover and pay for useful APIs with zero integration friction

## v1 Requirements

Requirements for milestone v1.0 — Publish + Brand. Each maps to roadmap phases.

### Package Hardening

- [x] **PKG-01**: `files` whitelist in package.json limits published content to `dist/`, `README.md`, `LICENSE`
- [x] **PKG-02**: `prepublishOnly` script runs `tsc` build before every publish
- [x] **PKG-03**: `engines` field declares Node 18+ requirement
- [x] **PKG-04**: LICENSE file exists on disk (MIT)
- [x] **PKG-05**: Shebang (`#!/usr/bin/env node`) preserved in `dist/index.js` after compilation
- [x] **PKG-06**: `publint` validates package exports before publish

### Input Validation

- [x] **VAL-01**: `coin` parameter validated with `/^[A-Z0-9]{1,10}$/i` regex
- [x] **VAL-02**: `url` and `pdf_url` parameters validated with `z.string().url()`

### npm Publish

- [x] **NPM-01**: README updated with npm install instructions and Claude config example (using `npx -y`)
- [x] **NPM-02**: Package published to npm registry as `x402-mcp-server`

### Brand Site — Marketing

- [x] **SITE-01**: Hero section with one-liner pitch and value proposition
- [x] **SITE-02**: Pricing table showing all tools with per-call costs
- [x] **SITE-03**: "How it works" section explaining x402 payment flow
- [x] **SITE-04**: OG meta tags for link sharing

### Brand Site — Documentation

- [x] **DOCS-01**: Getting started guide with free mode and paid mode paths
- [x] **DOCS-02**: API reference for all 6 MCP tools (params, returns, examples)
- [x] **DOCS-03**: Claude/MCP config example (copy-pasteable JSON)
- [x] **DOCS-04**: Wallet setup guide (Base network, USDC funding)

### Deployment

- [x] **DEPLOY-01**: Astro site builds to static output
- [x] **DEPLOY-02**: Site deployed to home server (no domain, IP/subdomain for now)

## v2 Requirements

Deferred to future milestones. Tracked but not in current roadmap.

### Developer Platform

- **DEV-01**: Interactive API playground on brand site
- **DEV-02**: SDK / client library for programmatic access
- **DEV-03**: Usage dashboard / analytics

### Infrastructure

- **INFRA-01**: Custom domain with SSL for brand site
- **INFRA-02**: CI/CD pipeline for automated npm publish
- **INFRA-03**: Uptime monitoring page

## Out of Scope

| Feature | Reason |
|---------|--------|
| User accounts / API keys | Private key IS the account — Phase 3 |
| Third-party API hosting | Phase 3 — developer platform |
| Own L2 chain | Phase 4 — far future |
| Mobile app | Not planned |
| Interactive API playground | Requires backend proxy — v2 |
| Automated CI/CD publish | Manual publish for v1.0 — adds secrets complexity |

## Traceability

Which phases cover which requirements. Updated during roadmap creation.

| Requirement | Phase | Status |
|-------------|-------|--------|
| PKG-01 | 1 — Package Hardening + Input Validation | Complete |
| PKG-02 | 1 — Package Hardening + Input Validation | Complete |
| PKG-03 | 1 — Package Hardening + Input Validation | Complete |
| PKG-04 | 1 — Package Hardening + Input Validation | Complete |
| PKG-05 | 1 — Package Hardening + Input Validation | Complete |
| PKG-06 | 1 — Package Hardening + Input Validation | Complete |
| VAL-01 | 1 — Package Hardening + Input Validation | Complete |
| VAL-02 | 1 — Package Hardening + Input Validation | Complete |
| NPM-01 | 2 — npm Publish | Complete |
| NPM-02 | 2 — npm Publish | Complete |
| SITE-01 | 3 — Brand Site Build | Complete |
| SITE-02 | 3 — Brand Site Build | Complete |
| SITE-03 | 3 — Brand Site Build | Complete |
| SITE-04 | 3 — Brand Site Build | Complete |
| DOCS-01 | 3 — Brand Site Build | Complete |
| DOCS-02 | 3 — Brand Site Build | Complete |
| DOCS-03 | 3 — Brand Site Build | Complete |
| DOCS-04 | 3 — Brand Site Build | Complete |
| DEPLOY-01 | 3 — Brand Site Build | Complete |
| DEPLOY-02 | 4 — Deployment | Complete |

**Coverage:**
- v1 requirements: 20 total
- Mapped to phases: 20
- Unmapped: 0 ✓

---
*Requirements defined: 2026-03-09*
*Last updated: 2026-03-09 — traceability table populated after roadmap creation*
